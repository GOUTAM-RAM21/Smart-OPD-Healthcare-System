...
export default function AarahiChat() {
  ...
  const SYSTEM_PROMPTS = {
    hi: "Tum Aarohi ho — ek friendly female health assistant. Sirf Hinglish (Hindi+English mix) mein baat karo. Warm aur caring tone rakho. Short answers do. Emojis use karo. Medical advice mat do.",
    en: "You are Aarohi — a friendly female health assistant. Speak only in English. Be warm and caring. Give short answers. Use emojis. Do not give medical advice.",
  }
  ...
  const sendMessage = useCallback((text) => {
    ...
    setTimeout(() => {
      const response = getAarahiResponse(text, SYSTEM_PROMPTS[lang])
            ...
            <div className="relative flex flex-col items-center pt-5 pb-4 px-4 overflow-hidden flex-shrink-0"
              ...
              <div className="absolute top-3 right-3 flex gap-2">
                <button onClick={() => setLang(l => l === 'hi' ? 'en' : 'hi')}
                  style={{ background: 'rgba(255,255,255,0.2)', color: 'white', border: '1px solid rgba(255,255,255,0.4)', borderRadius: 20, padding: '3px 10px', fontSize: 11, cursor: 'pointer' }}>
                  {lang === 'hi' ? 'EN' : 'HI'}
                </button>
